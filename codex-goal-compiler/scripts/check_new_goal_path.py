#!/usr/bin/env python3
"""Fail closed unless one explicit future GoalBuddy root does not exist."""
...
def check(path: str, project_root: str | None = None) -> dict[str, object]:
    ...
    expected_parent = root / "docs" / "goals"
    expanded = os.path.expanduser(str(path))
    if os.path.isabs(expanded):
        valid_shape = not has_malformed_components(expanded)
    else:
        parts = expanded.split("/")
        valid_shape = not has_malformed_components(expanded) and parts[:2] == ["docs", "goals"] and len(parts) == 3
    ...
    if not valid_location:
        return {
            ...
            "path": str(candidate),
            "collision": False,
            "error": f"goal root must be one direct docs/goals/<slug> child of {root}",
        }
    resolved_root = root.resolve(strict=False)
    if expected_parent.resolve(strict=False) != resolved_root / "docs" / "goals":
        return {
            "ok": False,
            "path": str(candidate),
            "collision": False,
            "error": "docs/goals must not traverse a symlinked ancestor",
        }
    ...
    return {
        ...
        "path": str(candidate),
        "collision": collision,
        "error": "goal root already exists; choose another slug without inspecting it" if collision else "",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Require a new, nonexistent GoalBuddy root.")
    parser.add_argument("path", help="Explicit future docs/goals/<slug> root.")
    ...
    if args.json:
        ...
        print(f"new goal root available: {report['path']}")
    else:
