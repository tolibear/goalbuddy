...
The checker consumes GoalBuddy compiler contract v1 and requires board schema v2 plus eight capabilities: `atomic_amendment_transition`, `atomic_placeholder_hydration_transition`, `lossless_receipt_identity`, `strict_multiline_yaml_projection`, `closed_judge_decision_vocabulary`, `atomic_exact_human_wait_resume`, `atomic_goal_completion`, and `task_bound_codex_exec_resume`. It also returns the exact absolute installed Goal Prep and compiler paths plus installed/source tree fingerprints. Missing requirements or mismatched bytes block preflight; additive capabilities do not. GoalBuddy itself owns doctor topology, installed agent inventory, compiler/backend presence, target readiness, source provenance, and installed skill binding. Agent-file presence alone is insufficient because the deterministic typed transition runtime must be able to apply either Judge outcome without a partial task edit/close/activate sequence, preserve receipt identity, enter and resume an exact-human wait through the digest-bound atomic surface, complete a goal through one digest-bound final-audit transition, resume an interrupted external Codex Worker by exact task-bound session identity, and fail closed at strict parser and decision boundaries. If preflight fails, block compilation and report the exact repair. Do not infer readiness from another harness or fall back to private schema.
