...
compatibility: "Codex with native /goal support for standalone goals; Python 3 for bundled validators; GoalBuddy compiler contract v1 and installed Goal Prep for board compilation; Claude Code supported for direct/GoalBuddy routing or later Codex handoff."
...
- Require GoalBuddy compiler contract v1 with board schema v2 and the required runtime capability subset for atomic amendment, atomic placeholder hydration, lossless receipt identity, strict multiline YAML projection, closed Judge decisions, atomic exact-human wait/resume, and atomic goal completion. Treat missing required capabilities as blocked and accept additive capabilities owned by newer compatible runtimes.
- Report the package version separately from the resolved CLI path, source kind, Git HEAD, and dirty state; a local dirty checkout is never a pristine published package claim.
- Use a new `docs/goals/<slug>` root and the future-only path guard.
- Treat the selected harness's installed Goal Prep skill as an explicit internal dependency whose implicit/model invocation may be disabled. After selecting GoalBuddy, explicitly load and execute its `SKILL.md` directly in the current compiler context; do not rely on implicit skill matching. Goal Prep owns board schema. Never spawn a subagent, collaboration agent, or separate Codex task merely to prepare the board.
...
- Start only when explicitly requested and the target-specific start gate passes.
