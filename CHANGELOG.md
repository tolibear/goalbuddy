...
- **Board keeping is now a first-class low-context control-plane role.** Codex and Claude Code installs include `goal_keeper` (GPT-5.6 Sol low) and `goal-keeper` (Opus 4.8 low). The PM retains every semantic decision while Keeper performs full-board inspection, receipt application, status/`active_task` transitions, task-card and gate updates, and board-health repair from a compact digest-bound request. Keeper validates every mutation with the existing checker, prefers the atomic receipt applier, returns `goalbuddy_keeper_receipt_v1`, and never becomes a task actor or product-file writer. One warm Keeper may be reused per board during an uninterrupted session; genuine recovery still starts with the separate read-only Ledger Auditor. Existing board schemas and historical receipts require no migration.
