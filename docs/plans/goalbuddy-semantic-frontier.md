...
- [x] (2026-07-30) Closed the completion audit against exact clean implementation commit `37efb253a2559ee77e37ff931914d2cd6b41f793`: added the missing M3 resume-output identities, repeated both compact Fable journeys against that commit, measured the actual legacy paths against the promoted paths, recorded review-yield and post-acceptance-defect fields, and repaired the PM deviation-evidence wording contradiction without changing runtime behavior.
...
- Observation: Promotion evidence must be bound to the exact clean commit being accepted.
  Evidence: the first Milestone 5 artifact described a precommit dirty tree. The completion audit repeated both compact journeys first against clean `115617b` and then against the exact corrected implementation commit `37efb253a2559ee77e37ff931914d2cd6b41f793`; the final record proves the candidate commit and status were unchanged before and after both runs.

- Observation: The promoted interface reduces observed control-plane work, not only prompt size.
  Evidence: the actual legacy non-UI and UI paths used 15 and 13 total tools, three GoalBuddy calls, and one manual digest relay each. The commit-bound compact paths used ten tools, two GoalBuddy calls, and no manual digest relay while preserving all eleven decisions, receipts, product bytes, checker-valid recovery, and zero reopened or post-acceptance defects.

- Observation: Phase-gate deviation instructions must use fields admitted by the receipt contract.
  Evidence: the compact kernel told PM/Judge to place phase-gate deviations in `deviations`, but the shared contract correctly reserves that field for Workers. The synchronized kernel now directs PM/Judge phase-gate reasons to `evidence` and reserves terminal `accepted_deviations` plus exact owner acceptance for unmet outcome requirements; policy tests lock that wording under the 18,000-byte kernel cap.
...
- Decision: Bind final promotion evidence to one exact clean implementation commit and compare it with observed legacy-path executions.
  Rationale: A dirty-tree identity and projected call-count claim are insufficient closeout evidence. Repeated no-session journeys at a stable commit plus actual old-path runs make both semantic parity and mechanical reduction falsifiable.
  Date/Author: 2026-07-30 / Codex

- Decision: Record phase-gate deviations in Judge/PM `evidence`, and use terminal `accepted_deviations` only for unmet outcome requirements with exact owner acceptance.
  Rationale: This keeps the execution doctrine consistent with the closed receipt schema and prevents either invalid PM receipts or diary-like Worker skip notes.
  Date/Author: 2026-07-30 / Codex
...
The completion audit then repeated the accepted compact pair against exact clean implementation commit `37efb253a2559ee77e37ff931914d2cd6b41f793` and ran the actual legacy paths for the same fixtures. Non-UI execution fell from 15 to 10 total tools and UI execution from 13 to 10; both fell from three to two GoalBuddy calls and from one manual digest relay to zero. Both compact runs retained all eleven decisions, exact reviewed product and evidence bytes, source receipts, checker-valid T001→T002 recovery, and no reopened or post-acceptance defect. The only post-Milestone-5 source correction aligned PM/Judge deviation instructions with the existing closed receipt schema; runtime scripts were unchanged.

Plan reconciliation remains complete: this is the only surviving GoalBuddy implementation plan; the useful task-level brief-binding and runtime-correctness work has been absorbed; the receipt index, owner profile/evaluation program, and monitor have been retired; and the closeout design now matches actual run provenance. The retained evidence records the change in Fable-visible GoalBuddy calls and input volume together with plan, prompt, review, screenshot, scope, deviation, and completion decisions. Final completion has demonstrated reduced mechanical burden, truthful terminal semantics, exact-current-review proof when required, and no evidence of reduced product quality.
...
- Branch: `codex/goalbuddy-semantic-frontier-m0`; exact evaluated implementation commit: `37efb253a2559ee77e37ff931914d2cd6b41f793`.
...
- Semantic comparison: `/tmp/goalbuddy-m3-fable-parity/comparison-record.json` (SHA-256 `1ce7ed40cb3a0a32e84757fb48c92e3af24b5b263e0790d11d93b5fb7f784dd6`) identifies both current resume outputs and both frontier outputs; parity remains 11/11 with 75.23% and 79.47% compact-input reductions.
- Commit-bound Fable promotion pair: `/tmp/goalbuddy-m5-fable-journeys-head-37efb25/comparison-validation.json` (SHA-256 `755197b9de1590935300068b3a3098a371d3afadb9e1744f60f01c94ebff31c7`) records non-UI and exact-render UI no-session journeys against exact clean commit `37efb253a2559ee77e37ff931914d2cd6b41f793`. Each uses ten total tools, exactly two GoalBuddy calls (`frontier`, `advance`), zero retries, zero manual control events, and zero forbidden calls; both preserve all eleven decisions, source receipts, product and review bytes, checker-valid T001→T002 recovery, and direct decisive-image inspection for the UI case.
- Quality comparison: `/tmp/goalbuddy-m5-fable-journeys-head-37efb25/quality-comparison.json` (SHA-256 `61dbbdd7c455e01898ea7abd4e8b72d486ef9c3298f755838d15e40422e2edb3`) compares observed legacy paths with the compact paths. Non-UI tools fall 15→10 and UI tools 13→10; GoalBuddy calls fall 3→2 and manual digest relays 1→0 in both; plan, prompt, diff, review, screenshot, scope, deviation, gate, receipt, and acceptance decisions are preserved; review yield is recorded; reopened and post-acceptance defect lists are empty.
...
This evidence record contains the final branch and evaluated implementation commit, test totals, package dry-run result, Codex and Claude doctor results, strict-projectability proof, provenance fixture disposition, accepted-deviation and exact-review proof, shadow-comparison disposition, observed legacy-path comparison, and fresh-Fable journey disposition.
...
2026-07-30: Completed the final evidence and documentation audit after Milestones 4-5. The audit replaced dirty-tree promotion evidence with two no-session canaries bound to exact clean implementation commit `37efb253a2559ee77e37ff931914d2cd6b41f793`, added the omitted M3 resume-output identities, ran actual legacy-path comparisons, and recorded review yield plus reopened/post-acceptance defect fields. The observed interface reduction is 15→10 and 13→10 total tools, 3→2 GoalBuddy calls, and 1→0 manual digest relays with all eleven decisions preserved.

2026-07-30: Reconciled the compact execution kernel with the closed receipt contract. PM/Judge phase-gate deviation reasons now belong in receipt `evidence`; unmet terminal requirements use `accepted_deviations` and exact owner acceptance. The change is synchronized to the plugin mirror, policy-tested under the 18,000-byte kernel cap, and does not alter runtime scripts. Full validation remains 400 Node tests plus 49 Python tests, a byte-exact mirror, and a 169-file package preview; live activation remains separately owner-gated.
