...
- Codex dispatch closes stdin. It defaults to `gpt-5.6-sol`, medium, Fast, and role sandboxing; `--reasoning-effort low|medium|high|xhigh|max|ultra` and `--service-tier default|flex` override per launch. It may bind its exact JSONL session id to the active task. Never use `codex exec resume --last`; resume requires proof the prior process is no longer live.
