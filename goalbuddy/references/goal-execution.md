...
- If the target CLI is missing, unauthenticated, returns a terminal error, or is confirmed unavailable, fall back to the normal path: PM fallback or the required GoalBuddy agent, per the dispatch rules above. A single timeout is an observation window, not proof of failure. An external Claude Opus architecture review must use a timeout of at least 1200 seconds.
...
### Durable Subagent Wait Contract

One `wait_agent` timeout ends only that observation window. After every timeout, inspect the actual `agent_status` and delivered messages before deciding what to do. If the agent is still running and has not reported an explicit error, unavailability, scope violation, or help request, continue bounded observation windows and give the user a short update during long waits.

Scout and Judge are read-only: file changes are never their progress signal. For a Worker, allowed-file changes may be evidence of execution, but their absence after one observation window is not proof of failure. Do not interrupt a Scout or Judge before 20 minutes, or a Worker before 30 minutes, unless there is an explicit error, confirmed unavailability, scope violation, or help request.

Interrupt only for a terminal error, confirmed unavailability, scope violation, or after multiple observation windows plus evidence of no execution. Never replace a still-running agent with PM fallback merely because one wait timed out; fallback requires proven failure under this contract. For external Claude Opus architecture reviews, set each wait timeout to at least 1200 seconds.
