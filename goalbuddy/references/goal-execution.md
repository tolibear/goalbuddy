...
- Codex dispatch closes stdin before launch and pins `gpt-5.6-sol`, medium reasoning, fast service tier, and read-only or Worker danger-full-access sandboxing instead of inheriting ambient config drift. It may atomically bind its exact JSONL session id to the active task. Never use `codex exec resume --last`; exact resume also requires proof the prior process is no longer live.
- Exact `codex exec resume <SESSION_ID>` starts a later turn; it is not same-turn steering. Codex app-server exposes experimental `turn/steer` and `turn/interrupt` for deep integrations, but this stable dispatcher does not invent an app-server daemon, process registry, or second ledger. If same-turn control is essential, use a separately approved app-server integration rather than pretending resume is equivalent.
