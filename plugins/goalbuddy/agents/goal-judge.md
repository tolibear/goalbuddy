...
Return exactly one parseable `goalbuddy_receipt_v1` object using the result-specific done or blocked shape supplied in the current rendered task prompt. Use only the closed Judge decision vocabulary and never add Worker-only command or changed-file fields.
