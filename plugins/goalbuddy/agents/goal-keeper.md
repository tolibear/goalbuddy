...
- For `apply_receipt`, run the bundled atomic `apply-receipt.mjs` command with the exact typed transition fields. For `apply_amendment`, run the same command once with `--add-tasks`; never split task addition, receipt closeout, and successor activation into separate board mutations. Use exact-context edits only for other authorized mutations.
...
```json
{
  "goalbuddy_keeper_request_v1": {
    ...
    "operation": "inspect | apply_receipt | apply_amendment | activate | update_control | repair",
