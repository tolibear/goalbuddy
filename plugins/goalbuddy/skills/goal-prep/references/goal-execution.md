...
- Codex dispatch closes stdin and pins `gpt-5.6-sol`, medium reasoning, role sandboxing, and Fast by default; `--service-tier default|flex` changes the tier. It may bind its exact JSONL session id to the active task. Never use `codex exec resume --last`; resume requires proof the prior process is no longer live.
