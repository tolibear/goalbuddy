...
- Codex dispatch closes stdin, pins `gpt-5.6-sol`, medium reasoning, fast service tier, and role-appropriate sandboxing, and may bind its exact JSONL session id to the active task. Never use `codex exec resume --last`; resume requires proof the prior process is no longer live.
