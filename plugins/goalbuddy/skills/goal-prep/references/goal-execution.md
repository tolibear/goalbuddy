...
- Route by explicit choice, task `harness:`, then loaded policy. Under Claude, Fable plans/reviews and implementation Workers default directly to Codex Exec; under Codex, implementation defaults to native Codex Workers. Do not wrap Codex in Opus. Keep routes out of board strategy.
- The dispatcher admits only the checker-validated active task at the expected digest, rereads before launch, runs the target CLI, extracts `goalbuddy_receipt_v1`, and checks a content-aware before/after manifest. Net Worker changes must exactly match `allowed_files` and receipt `changed_files`. It fails closed on control, out-of-root, duplicate, unchanged, out-of-scope, or read-only-role writes; fingerprints expose second edits to dirty files.
- Codex dispatch closes stdin. It defaults to `gpt-5.6-sol`, medium, the default service tier, and role sandboxing; `--reasoning-effort low|medium|high|xhigh|max|ultra` and `--service-tier fast|flex` override per launch. It may bind its exact JSONL session id to the active task. Never use `codex exec resume --last`; resume requires proof the prior process is no longer live.
- No implicit dispatch deadline applies. Use `--timeout <seconds>` only for an actual bounded runtime; never kill a healthy Worker on a generic timer.
- Exact `codex exec resume <SESSION_ID>` starts a later turn, not same-turn steering. The stable dispatcher adds no app-server daemon, process registry, or second ledger; approve a separate app-server integration if `turn/steer` or `turn/interrupt` is essential.
