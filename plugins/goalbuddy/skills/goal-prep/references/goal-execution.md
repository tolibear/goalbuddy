...
Complete canonical decisions use the deterministic digest-bound CLI directly: receipt closeout plus successor activation, amendment, hydration, exact-human wait/reply, held-receipt preservation, and final completion. These transitions validate under lock, install atomically, and return the new opaque digest plus the next relevant commands.
