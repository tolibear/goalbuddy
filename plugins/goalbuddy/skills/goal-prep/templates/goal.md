...
During `/goal` execution, the PM owns meaning. The PM may directly apply one already-known, one-location scalar or one-line annotation only when the compact projection or latest validated receipt supplies the exact old and new values and no board read is needed; use the current digest, an exact-context edit, and the bundled checker. If the PM must or may need to read, search, query, or inspect the board, or if a change touches multiple locations, receipts, task cards, scope, authority, approval, completion, or any uncertain transition, use `goal_keeper` in Codex or `goal-keeper` in Claude Code. Send one compact `goalbuddy_keeper_request_v1` with the current digest, exact operation, authorized control files, expected before/after facts, and bundled checker command. Reuse one Keeper within an uninterrupted session; its `after_digest` chains into the next Keeper operation. Full-board PM review is reserved for the execution contract's explicit recovery escalation.
...
3. Keep the Ledger-approved digest. Start a fresh Keeper only when board reading or a nontrivial mutation is needed; during uninterrupted Keeper transitions, reuse it and chain each validated `after_digest`. Do not rerun recovery or load the full board into PM context.
...
10. Apply only an execution-contract-qualified one-location mutation directly; otherwise give the exact mutation to Keeper. Continue only after the bundled checker passes and a new digest is recorded.
