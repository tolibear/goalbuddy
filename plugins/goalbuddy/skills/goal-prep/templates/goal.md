...
3. Keep the Ledger-approved digest. Apply complete canonical typed transitions directly and chain their validated `after_digest`. Start a fresh Keeper only when exceptional board reading, repair, rebinding, or noncanonical mutation is needed; during uninterrupted Keeper operations, reuse it and chain each validated `after_digest`. Do not rerun recovery or load the full board into PM context.
...
10. Apply a complete canonical typed transition directly through the atomic CLI, use the execution-contract-qualified one-location path only for a known trivial edit, and reserve Keeper for exceptional inspection, repair, rebinding, ambiguity, or noncanonical mutation. Continue only after the bundled checker passes and a new digest is recorded.
