...
The PM owns board meaning. Scout, Judge, and Worker return receipts; they do not select the next active task or mark the goal complete. During `/goal` execution, complete canonical typed decisions go directly through GoalBuddy's digest-bound atomic CLI; Board Keeper handles exceptional inspection, repair, rebinding, ambiguity, and noncanonical control work so the full board stays out of PM context. Newly applied receipts gain transition-owned adjacent provenance rather than agent-authored receipt fields. The PM may use the typed hold transition to preserve one exact unapplied candidate across interruption without accepting it or changing task status. Receipt shapes and all other runtime rules live in `references/goal-execution.md`.
