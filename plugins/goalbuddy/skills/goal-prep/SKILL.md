...
This document is the prep-mode contract plus the shared board model. The `/goal` execution contract lives in `references/goal-execution.md` next to this file; read it at the start of every `/goal` run. If it cannot be read, prepared execution fails closed without dispatch or mutation; do not reconstruct it from this skill or memory.

For preparation, each board has at most one active task, and explicitly requested parallel writers use depth-one child boards.

A healthy prepared run reads only the charter, compact kernel, and `goalbuddy_frontier_v1`; ordinary reviewed closeout uses explicit-source `advance`, while Keeper remains exceptional and Ledger remains recovery-only. For material work, Fable/PM owns slice strategy, current-repository research when useful, JIT ExecPlan and Codex operator-prompt approval, full product-diff review, independent-review selection and adjudication, direct decisive-screenshot inspection, unexpected-write/scope decisions, review convergence, accepted-deviation judgment, and final acceptance. Checker, test, Worker, receipt, or native task completion is evidence, not semantic completion. Claude's native task list is optional ephemeral projection, never board truth; rebuild it from the frontier on conflict. Routine successful control mechanics stay backstage; report product progress, reviews, blockers, decisions, and completion evidence.
...
The PM owns board meaning. Scout, Judge, and Worker return receipts; they do not choose the next task or completion. Ordinary closeout goes through explicit-source `advance`: the PM selects the reviewed source or held handle, authority, successor, and optional task card, while GoalBuddy derives provenance and returns the next frontier. Other canonical transitions remain digest-bound; Board Keeper handles only exceptional inspection, repair, rebinding, ambiguity, and noncanonical control work. Newly applied receipts gain transition-owned adjacent provenance. The typed hold transition preserves one exact unapplied candidate without accepting it or changing status. Runtime rules live in `references/goal-execution.md`.
...
tasks:
  ...
  - id: T999
    ...
    expected_output:
      ...
      - "accepted_deviations: complete ordered set"
      - "deviation_acceptance: persisted whole-set owner reply locator or null"
      - "final_review: exact-current complete review or accepted exact-final-review deviation"
    receipt: null
```
...
Keeper and Ledger are not represented in `state.yaml` agent availability or task cards. During execution, ordinary receipt closeout and activation use explicit-source `advance`; amendments, standalone hydration, exact-human wait/reply, optional held-receipt preservation, and final completion use their canonical digest-bound transitions. A held receipt is recoverable evidence, not applied status or semantic acceptance. Final completion requires either an exact-current complete review covering the root/child Worker path ledger, or one persisted owner reply binding the complete ordered deviation set; only an accepted `exact-final-review` deviation may replace the review. Every sibling and child board must be done, and the checker revalidates proof after completion.

Invoke `goal_keeper` (Codex, Sol low) or `goal-keeper` (Claude Code, Opus low) only when the PM must inspect board content beyond the semantic frontier, state is ambiguous, repair or runtime rebinding is required, or no typed transition represents the decision. Ordinary closeout never goes through Keeper. One already-known one-location scalar or annotation may be edited directly under the execution contract's digest, exact-context, and checker rules without reading the board. Keeper receives an exact request and current digest, makes no semantic decisions, and never edits product files.

At genuine new-session or post-compaction uncertainty or another recovery boundary, `/goal` runs the bundled `scripts/resume-board.mjs` entrypoint, then invokes `goal_ledger` (Codex, Sol medium) or `goal-ledger` (Claude Code, Opus high) with the board path, resume-response digest, checker status, and returned `commands.resume` command. Ledger independently reruns that same bundled entrypoint and compares the complete board with independent repo/worktree/receipt/verification/gate/Worker evidence; it never depends on a globally installed GoalBuddy CLI. It returns `goalbuddy_ledger_audit_v1`, never a task receipt, and never mutates the board. Only `congruent` with matching pre/post digests after an `ok: true` projection permits automatic continuation. Checker or strict-parser failure routes to full-board PM review without rewriting immutable completed-task history merely for checker conformance. After that review explicitly authorizes immutable-history compatibility, render only the current active task with the bundled prompt script plus `--expected-state-digest <sha256> --allow-immutable-history`; this digest-bound path reuses the transition proof and strict-parses exact active-task and top-level control bytes rather than the lossy UI fallback. The exact Keeper and recovery contracts live in `references/goal-execution.md`.

A task's `assignee` determines the agent. The task card is the order. The receipt is the return format.

Only the main `/goal` PM may choose the active task, semantic closeout, successor, deviation judgment, or completion. It uses `advance` for ordinary reviewed closeout and canonical typed transitions for other decisions; Keeper reads the board only for exceptional inspection, repair, rebinding, ambiguity, or noncanonical control work. The exact execution policy lives in `references/goal-execution.md`.
