...
async function resume() {
  ...
  const { createBoardPayload, normalizeGoalBoard, parseGoalStateText } = await import(boardLib);
  const explicit = positional(1);

  if (explicit) {
    const goalDir = resolve(explicit);
    ...
    try {
      const projection = createResumeProjection(
        goalDir,
        parseGoalStateText,
        normalizeGoalBoard,
        checker,
        validation.stateText,
      );
      if (hasFlag("--json")) {
...
function runResumeChecker(goalDir) {
  ...
  const script = join(skillSource, "scripts", "check-goal-state.mjs");
  const result = spawnSync(process.execPath, [script, goalDir], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: process.env,
  });
  ...
  try {
    ...
    // The synthesized failure below preserves stderr/stdout without trusting malformed checker output.
  }
  ...
  return {
    ...
    stateText: stateBefore !== null && stateBefore === stateAfter ? stateAfter : null,
  };
}
...
function createResumeProjection(goalDir, parseGoalStateText, normalizeGoalBoard, checker, stateText) {
  ...
  const runCommand = `/goal Follow ${path}/goal.md.`;
  const resumeCommand = `goalbuddy resume ${path} --json`;
  const promptCommand = activeTask ? `goalbuddy prompt ${path}` : null;
...
function projectCommands(commands) {
  ...
  return values.map((command) => {
    ...
    return {
      cmd: resumeText(command?.cmd),
      status: resumeText(command?.status),
    };
