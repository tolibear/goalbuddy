...
After recovery, GoalBuddy's Board Keeper (`goal_keeper` in Codex, `goal-keeper` in Claude Code) handles full-board inspection and every nontrivial execution-time board mutation. The PM may directly apply only one already-known scalar or one-line annotation when no board read is needed, using the current digest, an exact-context edit, and an immediate checker run. Receipts, task cards, status-plus-successor transitions, scope, authority, approvals, completion, multi-location changes, and uncertain state still require Keeper. The PM sends only an exact semantic decision and the current digest; Keeper reads the large board in its isolated context, applies the change, runs the existing checker, and returns a compact `goalbuddy_keeper_receipt_v1`. Keeper uses Sol low in Codex and Opus low in Claude Code. It never selects work or edits product files, and the separately read-only Ledger remains the independent recovery check.
